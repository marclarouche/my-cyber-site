...
import { Shield, Eye, AlertTriangle, Zap, ChevronRight, CheckCircle, XCircle, MinusCircle, BarChart2, Printer, ArrowLeft } from 'lucide-react';
...
@media print {
  ...
  .print-card { border: 1px solid #ccc !important; background: #fff !important; border-radius: 4px; margin-bottom: 12px; padding: 12px; }
  .print-score-box { border: 2px solid #333 !important; background: #f5f5f5 !important; color: #000 !important; }
  .print-answer-yes { background: #d1fae5 !important; color: #065f46 !important; border: 1px solid #6ee7b7 !important; padding: 2px 8px; border-radius: 4px; font-weight: bold; font-size: 12px; }
  .print-answer-partial { background: #fef3c7 !important; color: #92400e !important; border: 1px solid #fcd34d !important; padding: 2px 8px; border-radius: 4px; font-weight: bold; font-size: 12px; }
  .print-answer-no { background: #fee2e2 !important; color: #991b1b !important; border: 1px solid #fca5a5 !important; padding: 2px 8px; border-radius: 4px; font-weight: bold; font-size: 12px; }
  .print-answer-na { background: #f1f5f9 !important; color: #475569 !important; border: 1px solid #cbd5e1 !important; padding: 2px 8px; border-radius: 4px; font-weight: bold; font-size: 12px; }
  .print-answer-unanswered { background: #f8fafc !important; color: #94a3b8 !important; border: 1px solid #e2e8f0 !important; padding: 2px 8px; border-radius: 4px; font-size: 12px; }
  .print-section-header { background: #1e293b !important; color: #fff !important; padding: 8px 12px; border-radius: 4px; margin-bottom: 8px; }
  .print-risk-box { background: #fff1f2 !important; border: 2px solid #fca5a5 !important; color: #000 !important; }
...
function ResultsPanel({ answers, onBack }) {
  ...
  const answerLabel = (val) => {
    if (!val) return { text: "—  Not answered", cls: "print-answer-unanswered" };
    if (val === "yes")     return { text: "✓  Yes",     cls: "print-answer-yes" };
    if (val === "partial") return { text: "◑  Partial", cls: "print-answer-partial" };
    if (val === "no")      return { text: "✗  No",      cls: "print-answer-no" };
    if (val === "na")      return { text: "—  N/A",     cls: "print-answer-na" };
    return { text: val, cls: "print-answer-unanswered" };
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8 no-print">
        <button
          onClick={onBack}
          className="text-slate-400 hover:text-cyan-400 transition-colors text-sm flex items-center space-x-1"
        >
        ...
        <button
          ...
          className="flex items-center space-x-2 px-5 py-2.5 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-600 hover:border-cyan-500/50 text-slate-200 hover:text-cyan-300 transition-all text-sm font-semibold"
        >
      ...
      <div className="print-only" style={{marginBottom:'20px'}}>
        <h1 style={{fontSize:'22px',fontWeight:'bold',marginBottom:'4px'}}>AI Coworker Security Self-Assessment Report</h1>
        <p style={{fontSize:'12px',color:'#64748b'}}>Generated {new Date().toLocaleDateString('en-US', { year:'numeric', month:'long', day:'numeric' })}</p>
      ...
      {/* Full Answers Detail — visible on screen and in print */}
      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 mb-6 print-card">
        <h3 className="text-lg font-bold mb-6 flex items-center space-x-2">
          <CheckCircle className="w-5 h-5 text-cyan-400" />
          <span>Complete Assessment Responses</span>
        </h3>
        <div className="space-y-8">
          {SECTIONS.map((section) => {
            ...
            return (
              <div key={section.id}>
                <div className="flex items-center space-x-2 mb-4 pb-2 border-b border-slate-700 print-section-header">
                  <Icon className="w-4 h-4 text-cyan-400" />
                  <h4 className="font-semibold text-slate-100">{section.label}</h4>
                  <span className="ml-auto text-xs text-slate-500 font-mono">
                    {Math.round(calcSection(section, answers) * 100)}%
                  </span>
                </div>
                <div className="space-y-3">
                  {section.questions.map((q) => {
                    const val = answers[q.id];
                    const { text: aText, cls: aCls } = answerLabel(val);
                    const weightLabel = q.weight === 3 ? "High" : q.weight === 2 ? "Medium" : "Low";
                    const weightColor = q.weight === 3 ? "text-red-400" : q.weight === 2 ? "text-amber-400" : "text-slate-400";
                    return (
                      <div key={q.id} className="bg-slate-950/50 border border-slate-800 rounded-xl p-4 print-card">
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1 min-w-0">
                            <p className="text-slate-200 text-sm font-medium leading-relaxed mb-1">{q.text}</p>
                            <p className="text-slate-500 text-xs leading-relaxed">{q.hint}</p>
                          </div>
                          <div className="flex flex-col items-end gap-1.5 shrink-0">
                            <span className={`${aCls} text-xs font-semibold px-3 py-1 rounded-lg whitespace-nowrap`}>
                              {aText}
                            </span>
                            <span className={`text-xs ${weightColor} whitespace-nowrap`}>
      ...
      {/* CTA */}
      <div className="bg-gradient-to-br from-cyan-900/20 to-blue-900/20 border border-cyan-500/30 rounded-2xl p-8 text-center no-print">
      ...
      {/* Print-only CTA footer */}
      <div className="print-only" style={{marginTop:'20px', borderTop:'1px solid #e2e8f0', paddingTop:'12px', textAlign:'center'}}>
        <p style={{fontSize:'13px', color:'#0891b2', fontWeight:'bold'}}>Ready to close your gaps? Book a free consultation:</p>
        <p style={{fontSize:'12px', color:'#0891b2'}}>https://calendly.com/cyberlifecoach-proton/new-meeting</p>
      </div>
    </div>
  );
}
  ...
  if (showResults) {
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100">
        {/* Navigation */}
        <nav className="fixed w-full z-50 bg-slate-900/95 backdrop-blur-sm shadow-lg no-print">
